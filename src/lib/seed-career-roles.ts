/**
 * Seed script to populate the 3 MVP career roles
 * Run this script to initialize the career_roles table with prompts
 */
...
const MVP_ROLES = [
  {
    ...
    shortDescription: "Craft effective prompts to get better AI outputs for various business applications",
    ...
    isActive: true,
    sortOrder: 1,
    systemPrompt: `You are an expert AI Prompt Engineering career advisor with deep experience evaluating candidates for prompt engineering roles at AI companies, agencies, and enterprise organizations.
...
Begin now by introducing yourself and explaining the process.`,
  },
  {
    ...
    shortDescription: "Create high-quality content using AI tools while maintaining authenticity and brand voice",
    category: "Content & Communication",
    accessibilityLevel: "High",
    typicalTimeline: "1-6 months",
    ...
    isActive: true,
    sortOrder: 2,
    systemPrompt: `You are an expert AI Content Creation career advisor with extensive experience evaluating candidates for content creation roles using AI tools across marketing agencies, media companies, and content-focused businesses.
...
Begin now by introducing yourself and explaining the process.`,
  },
  {
    id: "ai-coach",
    name: "AI Coach",
    shortDescription: "Train teams and individuals on effective AI tool adoption for productivity",
    ...
    isActive: true,
    sortOrder: 3,
    systemPrompt: `You are an expert AI adoption and training career advisor with extensive experience evaluating candidates for AI Coach roles - the fastest-growing emerging AI position with +57.7% year-over-year growth.
...
export async function seedCareerRoles(): Promise<void> {
  try {
    ...
    for (const roleData of MVP_ROLES) {
      // Check if role already exists
      const existing = await roleRepo.findOne({ where: { id: roleData.id } });

      if (existing) {
        // Update existing role
        await roleRepo.update({ id: roleData.id }, roleData);
        console.log(`Updated career role: ${roleData.name}`);
      } else {
        // Create new role
        const role = roleRepo.create(roleData);
        await roleRepo.save(role);
        console.log(`Created career role: ${roleData.name}`);
      }
    }

    console.log("✅ Career roles seeded successfully");
  } catch (error) {
    console.error("❌ Error seeding career roles:", error);
    throw error;
  }
}

// Export for use in API route or standalone script
export default seedCareerRoles;
